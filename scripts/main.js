import { buildArcaneBacklashPacks } from "./packs/arcane-backlash.js";
import {
  isArcanePackEnabled,
  MODULE_ID,
  registerArcaneBacklashSettings
} from "./settings.js";
...
function notifyRegistrationFailure(error) {
  ...
  if (game.user?.isGM) {
    ui.notifications.error(
      game.i18n.localize("PF2ECFAB.Notifications.RegistrationFailed"),
      { permanent: true }
    );
  }
}
...
function currentPacks() {
  ...
  return buildArcaneBacklashPacks(isArcanePackEnabled);
}
...
function queuePackRefresh() {
  ...
  queueMicrotask(() => {
    ...
    try {
      ...
      if (game.user?.isGM) {
        ui.notifications.info(game.i18n.localize("PF2ECFAB.Notifications.PackSelectionUpdated"));
      }
    } catch (error) {
...
Hooks.once("init", () => {
  try {
    registerArcaneBacklashSettings(queuePackRefresh);
